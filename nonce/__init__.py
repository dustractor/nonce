...
default_mtrx = pkg_resources.resource_filename(__name__,"data/english.gibi")

class NonceMain(argparse.ArgumentParser):
    def __init__(self):
        super(NonceMain,self).__init__(prog="nonce")
        self.add_argument("--matrixfile",default=default_mtrx,
                metavar="path",
                help="Path to a gibi matrix file. DEFAULT: " + default_mtrx)
        self.add_argument("--archivefile",
                default=os.path.expanduser("~/.nonce.db"),
                metavar="path",
                help="Path to a specific archive. DEFAULT: ~/.nonce.db")
        self.add_argument("--title",action="store_true",
                help="""Capitalize the first letter of the first word"""
                    """ and add a period after the last word.""")
        self.add_argument("--last",action="store_true",
                help="Show the last word again.")
        self.add_argument("--dump",action="store_true",
                help="Show the words.")
        self.add_argument("--drop",action="store_true",
                help="Clear the archived history of words.")
        self.add_argument("--word",nargs="?",
                help="Add a word and return it.")
        self.add_argument("--words",nargs="?",type=int,default=1,
                metavar="N",
